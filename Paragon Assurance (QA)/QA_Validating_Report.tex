...
\end{tabular}
\caption{QA Audit Team --- Paragon Assurance}
\end{table}
...
The audit identified \textbf{seven (7) process non-compliance instances} across the three RE methods audited. No corrective actions or remediation steps are prescribed; findings are presented to Nevon Solutions for review and decision.
...
Low             & 1 & PA-ERR-007 (Grammatical Defect Uncorrected) \\\hline
\end{tabular}
...
\begin{itemize}
    ...
    \item A grammatical defect in requirement QA-USAB-01 was not corrected by editorial review (PA-ERR-007).
\end{itemize}

The QC team (Veridion Labs) produced a documented plan with 10 inspection techniques, a self-contained checklist of 21 items, 12 findings across all three required error types (Authorship, Omission, Contradiction), and a complete sign-off by all four team members. The QC process is assessed as compliant. A detailed QA audit of the QC Validating process is presented in Section~\ref{sec:qc-audit}.
...
PA-ERR-007 & SRS \S5.4.1 QA-USAB-01 & Double-predicate grammatical construction (``shall allow\ldots must be able to'') uncorrected. \\\hline
\end{tabular}
...
UR-MOB-06 mandates 2FA for parents and teachers (mobile), while NFR-SEC-03 mandates 2FA exclusively for administrators (web portal). These are directly contradictory. BC-01 and TC-02 session outcomes contain no reference to 2FA scope resolution. The QC team independently confirmed this contradiction (QC-FND-001).
...
Three-way inconsistency: UR-MOB-01 uses ``Attended, Absent, Late, or Excused''; \S4.3.2 uses ``Excused, Attended, Late, Absent''; Data Dictionary Table C.4 uses ``Present.'' This should have been detected and unified during the Specifying phase. The QC team independently confirmed this (QC-FND-002).
...
\subsection{PA-FND-007 --- Grammatical Defect in QA-USAB-01 Uncorrected}

\noindent\textbf{Error:} PA-ERR-007 \quad \textbf{Severity:} Low \quad \textbf{Phase:} Validating --- Editorial Quality Review

QA-USAB-01 reads: ``The system shall allow parent and teacher users \emph{must be able to} complete primary tasks\ldots'' --- a double-predicate construction producing a syntactically malformed requirement. The QC team independently confirmed this defect (QC-FND-009).
...
Three QC findings independently corroborate QA findings: QC-FND-001/PA-ERR-003 (2FA contradiction), QC-FND-002/PA-ERR-005 (terminology inconsistency), and QC-FND-004/PA-ERR-004 (enrolment omission).
...
QA Auditor & Zong Han & \includegraphics[width=3.5cm,height=0.8cm]{../Brigman/Signatures/signature_zonghan.jpg} & 19/03/2026 \\\hline
\end{tabular}
\end{table}
...
B.4 & Is terminology consistent across all SRS sections and artefacts? & M2 & NS & Attendance status terminology is inconsistent across SRS sections. UR-MOB-01 uses ``Attended, Absent, Late, or Excused''; \S4.3.2 uses ``Excused, Attended, Late, Absent.'' QC Report (QC-FND-002) confirmed ``Present'' in Data Dictionary Table C.4 diverges from both. See PA-ERR-005. \\\hline
...
C.3 & Is there evidence of a grammar check before SRS submission? & M3 & PS & No documentary evidence of a grammar check was found. QA-USAB-01 contains an uncorrected double-predicate grammatical defect (PA-ERR-007), suggesting grammar review was incomplete or not performed. \\\hline
C.4 & Is there evidence of an editorial review before SRS submission? & M3 & NS & No editorial check record found. The persistence of grammatical defects (PA-ERR-007) and terminology inconsistencies (PA-ERR-005) indicates editorial review was either not performed or was ineffective. \\\hline
\end{longtable}
...
D.2 & Are process artefacts internally consistent with SRS content? & M2 & PS & Context DFD documents six external entities consistent with SRS \S2.1. However, Data Dictionary Table C.4 uses ``Present'' inconsistent with SRS \S3.1 and \S4.3.2 (QC-FND-002). \\\hline
...
QC Report & QC-FND-001 & Independent confirmation; classified High severity \\\hline
\end{tabular}
\end{table}
...
QC Report & QC-FND-004 & Independent confirmation; classified High severity \\\hline
\end{tabular}
\end{table}
...
QC Report & QC-FND-002 & ``Attended'' vs ``Present'' in Data Dictionary Table C.4; classified High \\\hline
\end{tabular}
\end{table}
...
\subsection{PA-FND-007 --- Evidence}
...
\rowcolor{tableheader}
\textbf{Source} & \textbf{Location} & \textbf{Content} \\\hline
SRS & \S5.4.1 QA-USAB-01 & ``The system shall allow parent and teacher users must be able to complete primary tasks\ldots'' --- double predicate construction \\\hline
QC Report & QC-FND-009 & Independent confirmation; classified Low severity \\\hline
\end{tabular}
\end{table}
...
12 error instances: Contradiction (3: 2FA scope, attendance terminology, report generation time), Omission (5: biometric enrolment, password recovery, offline handling, session management, RBAC glossary), Authorship (4: periodic synchronisation, QA-USAB-01 grammar, capitalisation, unsigned sign-off).

\subsection{QC Findings Assessment}

12 findings with Context-Result-Analysis structure. Severity: High (5), Medium (4), Low (3). Three findings independently corroborate QA findings (QC-FND-001/PA-ERR-003, QC-FND-002/PA-ERR-005, QC-FND-004/PA-ERR-004).
