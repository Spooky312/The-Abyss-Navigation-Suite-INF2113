...
For readability in the main report, the consolidated error detection register is presented in \hyperref[tab:qc-error-detection]{Appendix C, Detected QC error instances}. The appendix table serves as the detailed supporting record for the findings discussed in Section~\ref{sec:qc-findings}.
...
\hline
QC-FND-001 & Contradiction & High & 2FA required for parents and teachers in UI requirements but only for administrators in security NFRs. \\
\hline
QC-FND-002 & Contradiction & High & Attendance status terminology inconsistent -- ``Attended'' vs ``Present'' used across sections and data dictionary. \\
\hline
QC-FND-003 & Contradiction & Medium & Report generation time stated without query-type distinction in stimulus/response, contradicting NFR-PERF-03. \\
\hline
QC-FND-004 & Omission & High & No requirement for biometric enrolment process -- how students are initially registered in the system. \\
\hline
QC-FND-005 & Omission & High & Account lockout specified but no password reset or account recovery flow requirement exists. \\
\hline
QC-FND-006 & Omission & High & Offline mode limit specified but no requirement for what happens when limit is exceeded or sync conflicts arise. \\
\hline
QC-FND-007 & Omission & Medium & Sessions referenced throughout but no requirement defining how sessions are created or managed. \\
\hline
QC-FND-008 & Authorship & Medium & ``Periodic synchronisation'' in Section 2.2 is vague and inconsistent with the specific 5-minute figure in FR-COM-01. \\
\hline
QC-FND-009 & Authorship & Low & Grammatical error in QA-USAB-01 -- double verb construction ``shall allow \ldots{} must be able to''. \\
\hline
QC-FND-010 & Authorship & Low & UR-WEB-08 and UR-WEB-09 use lowercase ``the portal shall'' inconsistent with all other requirements. \\
\hline
QC-FND-011 & Authorship & Low & SRS sign-off section contains entirely blank approver signatures and dates for all four designated approvers. \\
\hline
QC-FND-012 & Omission & Medium & RBAC used extensively throughout but not defined in Appendix A Glossary. \\
\hline
\end{tabularx}
