...
This protocol implements all three change mitigation strategies defined in
Specifying methodology: anticipatory contingency requirements, open issues,
and high-risk change control.

\subsection{8.1 Anticipatory Contingency Requirements}

The following dormant requirements are pre-approved and may be activated
without contract renegotiation if triggering conditions occur:

\begin{itemize}
\item CONT-REG-001: If maritime regulatory requirements introduce new
depth or reporting limits, system logging and reporting thresholds shall
be configurable without architectural redesign.

\item CONT-SENS-001: If customer-supplied sensor interface versions change,
the system shall support backward-compatible schema adapters.

\item CONT-COMM-001: If operational communication bandwidth reduces below
1 kbps baseline, system shall prioritize safety telemetry over mission data.
\end{itemize}

\subsection{8.2 Open Issues Register}

Open issues are requirements not fully resolved at SRS baseline and subject
...
\hline
Issue ID & Description & Responsible Party & Kill Date \\ \hline
OI-001 & Final database product selection & Vendor & 30 days post-baseline \\
OI-002 & Final JSON sensor schema approval & Customer & 45 days post-baseline \\
\hline
\end{tabular}
\caption{Open Issues Register}
\end{table}

Open issues unresolved by kill date shall default to baseline assumptions.

\subsection{8.3 High-Risk Change Control}

High-risk requirements include:

\begin{itemize}
\item All SAF-* requirements
\item COM-DEG-001
\item SEC-ENC-001, SEC-AUTH-001
\end{itemize}

Any proposed modification to high-risk requirements shall:

\begin{enumerate}
\item Undergo formal impact analysis (safety, schedule, cost)
\item Be reviewed by Customer Safety Approver
\item Be approved by Vendor Authenticator
\item Update iRTM and verification artifacts in the same revision
\end{enumerate}
