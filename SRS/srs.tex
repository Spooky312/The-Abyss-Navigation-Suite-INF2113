...
SAF-EMG-001 & Essential & BO-L1-4, BO-L2-4.1 & QUAL-SAF-001, QUAL-OPS-001 & The system shall detect pressure deviations \(>\pm5\%\) from nominal setpoint within \textbf{5 seconds}. & Test: Inject simulated pressure spike; measure detection latency. & \hyperref[fig:usecase-diagram-pdf]{UC10} (Detect Abnormal Conditions) \\
\hline
SAF-EMG-002 & Essential & BO-L1-4, BO-L2-4.1 & QUAL-SAF-001, QUAL-OPS-001 & The system shall detect temperature deviations \(>\pm10\%\) from nominal within \textbf{10 seconds}. & Test: Thermal chamber simulation or sensor-in-the-loop test. & \hyperref[fig:usecase-diagram-pdf]{UC10} (Detect Abnormal Conditions)\\
\hline
SAF-EMG-003 & Essential & BO-L1-4, BO-L2-4.1 & QUAL-SAF-001, QUAL-OPS-001 & The system shall detect power supply deviations \(>\pm15\%\) from nominal within \textbf{10 seconds}. & Test: Power supply fault injection; measure detection latency. & \hyperref[fig:usecase-diagram-pdf]{UC10} (Detect Abnormal Conditions)\\
\hline
SAF-STATE-001 & Essential & BO-L2-4.2 & QUAL-SAF-001 & Upon detection of any abnormal condition, the system shall transition the submersible to a \textbf{safe operational state} defined as neutral buoyancy, minimal power consumption, and active communication beacon. & Integration test: End-to-end emergency scenario. & \hyperref[fig:usecase-diagram-pdf]{UC09} (Auto-Trigger Retrival) \\
\hline
SYS-CONT-001 & Essential & BO-L1-5 & QUAL-RES-001, QUAL-SAF-001 & When non-critical degradation occurs, the system shall continue the mission in a degraded mode while alerting the operator and logging the degradation event. & Test: Inject non-critical fault; confirm degraded continuation and alert/log. & \hyperref[fig:usecase-diagram-pdf]{UC04} (Execute Mission Controls Override) + \hyperref[fig:usecase-diagram-pdf]{UC02} (Monitor Vessel Health) \\
\hline
...
DATA-LOG-001 & Essential & BO-L1-6, IT-L1-5 & QUAL-TRC-001 & The system shall log mission events and operator actions with timestamp (UTC), vehicle ID, and operator ID attribution. & Inspection/Test: Review log records for required fields. & \hyperref[fig:usecase-diagram-pdf]{UC13} (Log Operator Operations) + \hyperref[fig:usecase-diagram-pdf]{UC15} (Log Mission Data) \\
\hline
...
\hline
SEC-AUTH-001 & Essential & IT-L0-C3, IT-L1-4 & QUAL-SEC-001 & The system shall enforce mutual authentication for system-to-system communications using \textbf{X.509 certificates}. & Test: Reject invalid/expired certificates; accept valid certificates only. & \hyperref[fig:usecase-diagram-pdf]{UC08} (Synchronise Mission Data) \\
\hline
...
The BDST Quality Model v1.0 is derived from and conceptually aligned with the McCall Software Quality Model (McCall et al., 1977). While McCall classifies quality across \textbf{Product Operation}, \textbf{Product Revision}, and \textbf{Product Transition} perspectives, BDST adapts these principles to the safety‑critical, unmanned deep‑sea operational domain. The selected quality features operationalize McCall’s attributes in a context‑specific manner relevant to constrained communications, autonomous control, and mission accountability.
...
\begin{itemize}[leftmargin=*,nosep]
  ...
  \item McCall, J.A., Richards, P.K., \& Walters, G.F. (1977). \textit{Factors in Software Quality}. Rome Air Development Center.
\end{itemize}
